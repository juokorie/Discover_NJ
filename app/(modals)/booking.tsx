       ...
       <View style = {styles.container} >
            ...
            <View style = {styles.card}>
                ...
                {openCard === 0 &&(
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator = {false} contentContainerStyle ={{gap:25, paddingLeft: 20, marginBottom: 20}}>
                        {places.map((item, index) =>(
                            <TouchableOpacity key = {index} onPress={() => setSelectedPlace(index)}>
                               <Image source = {{uri: item.img}} resizeMode='contain' style = {selectedPlace === index ? styles.placeSelected: styles.place }/>
                               <Text style = {[{ paddingTop: 6, textAlign: 'center'}, selectedPlace === index ? {fontFamily: 'mon-sb'} : {fontFamily: 'mon'} ]}> {item.title}</Text>
                            </TouchableOpacity>
                        ))}
                    </ScrollView>
                    </>
